<template>
  <div>
    <div class="cdie" id="cdie">
      <canvas id="c" ref="canvas" style="z-index: 2"></canvas>
      <div class="selectarea" ref="selectArea"></div>
      <div class="copy"></div>
...
export default {
  ...
  methods: {
    ...
    initSelect() {
      //框选区域初始化
      this.selectobj = this.$refs.selectArea;
      this.selectobj.style.display = "none";
      const ctx = this.canvas.getContext("2d");
      console.log("initSelect:this.width this.height", this.width, this.height);
      if (this.temp) {
      ...
      canvas
        .on("mouse:down", (e) => {
          if (this.isSelecting) {
            ...
            console.log("初始坐标startPoint:", startPoint.x, startPoint.y);
            this.shapeSelect = new fabric.Rect({
              //创建对应图形类型
              left: startPoint.x,
              top: startPoint.y,
              width: 0,
              ...
              stroke: "#000",
              strokeDashArray: [5, 5],
              selectable: true, // 确保selectable属性为true
            });
            if (this.shapeSelect) {
              f.add(this.shapeSelect); //添加图形
            ...
            canvas
              .on("mouse:move", (e) => {
                // this.initSelect();
                // console.log("move:e", e.e);
                // var endx = e.e.offsetX;
                // var endy = e.e.offsetY;
                // console.log("move:endx, endy", endx, endy);
                // minXS = startx > endx ? endx : startx;
                // minYS = starty > endy ? endy : starty;
                // wS = Math.abs(endx - startx);
                // hS = Math.abs(endy - starty);
                console.log("select mousemove:");
                const p = f.getPointer(e.e) || {
                  x: 0,
                  y: 0,
                };
                minXS = Math.min(p.x, startPoint.x);
                minYS = Math.min(p.y, startPoint.y);
                wS = Math.abs(p.x - startPoint.x);
                hS = Math.abs(p.y - startPoint.y);
                console.log(
                  "select move:minXS, minYS, wS, hS",
                  minXS,
                  minYS,
                  wS,
                  hS
                );
                this.shapeSelect.set({
                  left: minXS,
                  top: minYS,
                  width: wS,
                  height: hS,
                ...
                // console.log("move:minXS, minYS, wS, hS", minXS, minYS, wS, hS);
                // var styleObj = {
                //   display: "block",
                //   left: minXS,
                //   top: minYS,
                ...
                // this.selectobj.style.left = minXS + "px";
                // this.selectobj.style.top = minYS + "px";
                // this.selectobj.style.width = wS + "px";
                // this.selectobj.style.height = hS + "px";

                // for (var key in styleObj) {
                //   this.selectobj.style[key] = styleObj[key];
                // }
              })
              .on("mouse:up", (e) => {
                ...
                // var w = this.select_width;
                // var h = this.select_height;
                // var minX = this.select_minX;
                // var minY = this.select_minY;
                console.log("minXS, minYS, wS, hS", minXS, minYS, wS, hS);
                this.temp = ctx.getImageData(minXS, minYS, wS, hS);
                ctx.clearRect(minXS, minYS, wS, hS);
                this.history.push(ctx.getImageData(0, 0, wS, hS));
                ctx.putImageData(this.temp, minXS, minYS);
                this.selectDrag(minXS, minYS, wS, hS, this.shapeSelect, canvas); //shape为框选区域的对象)
              ...
              switch (drawType) {
                case "r":
                  shape = new fabric.Rect({
                    ...
                    fill: undefined,
                    stroke: "red",
                    selectable: false, // 确保selectable属性为false
                  });
                  break;
                case "c":
                  {
        ...
        .on("mouse:up", (e) => {
          ...
          //   this.history.push(ctx.getImageData(0, 0, wS, hS));
          //   ctx.putImageData(this.temp, minXS, minYS);
          //   this.selectDrag(minXS, minYS, wS, hS, this.selectobj, canvas); //shape为框选区域的对象
          // } else {
          if (drawType && shape) {
    ...
    selectDrag(x, y, w, h, selectAreaObj, canvas) {
      //移动框选的区域内容
      ...
      var cx;
      var cy;
      console.log("selectDrag x, y, w, h:", x, y, w, h);
      const ctx = canvas.getContext("2d");
      canvas.on("mouse:down", (e) => {
        ...
        cx = ox - x;
        cy = oy - y;
        console.log("ox oy cx cy:", ox, oy, cx, cy);
        ...
        canvas
          .on("mouse:move", (e) => {
            ...
            if (this.history.length != 0) {
              console.log(
                "history不为空 true or false:",
                this.temp == this.history[this.history.length - 1]
              );
            ...
            x = left;
            y = top;
            console.log("selectArea:", left, "  ", top);
            ctx.putImageData(this.temp, left, top);
          })
          .on("mouse:up", (e) => {
            this.canvas.off("mouse:move");
            this.canvas.off("mouse:up");
      ...
      //     left = 0;
      //   }
      //   if (left > this.canvas.width - w) {
      //     left = this.canvas.width - w;
      //   }

      //   if (top < 0) {
      //     top = 0;
      //   }
      //   if (top > this.canvas.height - h) {
      //     top = this.canvas.height - h;
      //   }
      //   // selectAreaObj.css({
      //   //   left: left,
      //   //   top: top,
